...
- `sync-briefs-to-skills.sh` and `regenerate-standards-digest.py` were fixed on 2026-09-09 to read `SUPABASE_SERVICE_KEY` from the environment `[F84]`. Expect both to appear in this list anyway: the section 6 grep includes the pattern `SUPABASE_SERVICE`, and the fix put that variable name into both scripts `[ALIGN]` `[F84]`. A match here does not mean the fix failed, and a filename-only list cannot tell you either way; whether either script still holds the key in plaintext is not settled by this section, and gate 4.3a is where it gets read. `vps-heartbeat.sh` will match too, because its header comment contains the words `service_role` `[HBSH]`. `render-identity.py` is expected to match because it kept a hardcoded fallback `[F85]`.
...
- This is the only method on record for finding out what the host writes, because the scripts the crontab names are not checked in `[F37]` `[F38]`. The two host scripts that are checked into `ai-harness` do not change that: `n8n-exec-governor.py` holds no Supabase reference at all `[GOV]`, and `vps-heartbeat.sh` writes to GitHub, not Supabase `[F60]`.
...
2. Read `count` in the printed state file against the warn line of 7,000 `[F64]`. The last recorded count was 2,917 at 2026-09-19T09:05Z, eighteen days into the cycle that started 2026-09-01 `[HB]` (the facts file says twelve days at `[F76]`; the record's own dates give eighteen); the count was 1,680 on 2026-09-09 `[F70]`. A count that is far higher than that trend, or a `cycle_start` that is not the first of this month, is worth a line in your report.
...
5. If two things you saw disagree with each other or with this document, say so rather than picking one. Known disagreements on record are: `poll_sync_queue.py` cadence `[F115]`, the snapshot size `[F116]`, the gateway job count of ~38 versus 33 versus 45 `[F117]`, brief rendering `[F118]`, the governor's action `[F119]` and alert path `[F120]`, the `audit_critical_infra` cadence `[F121]`, `learning_events` `[F122]`, the factory switch versus the `format_aliases` ledger on where `mindmaker_live`, `techonomic` and `builder_economy` resolve (gate 4.1), and the VPS audit's decision table versus the wind-down rule on an absent governor (section 3.2). A new one is a finding, not a mistake.
