...
2. **Diverge before committing to a novel concept.** For a load-bearing surface with no approved pattern, generate independent concept spines, judge them adversarially in fresh context where available, and synthesize the strongest. On a post-rejection reset, enforce `krish-design`'s asymmetric context rule: generators receive a sanitized, non-leading brief without rejected solution artifacts; the independent judge receives the full relevant history. Then present one rendered synthesis to Krish unless comparison itself is the decision. One mock must not mean one unchallenged guess.
