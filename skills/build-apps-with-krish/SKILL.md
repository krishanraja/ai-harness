...
description: "Founder-in-the-loop orchestrator for a materially new app, product experience, or cross-surface redesign, from product rules through approved rendered mocks, implementation, and evidence-backed release. Use for a new or rebuilt app, several connected product surfaces, corpus-to-product work, or a multi-session app-delivery journey. Exclude isolated fixes, one specified component, diagnosis-only UX audits, non-app visuals, and routine work inside a locked design system; route those to `krish-build`, `krish-design`, or `ux-testing-agent`. Load `krish-principles` and `strategy-brief` first. Orchestrate, but never replace specialist skills or Krish's material visual and go-live approvals."
---
