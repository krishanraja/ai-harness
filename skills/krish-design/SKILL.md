...
When a rejected spine triggers fresh divergence, give independent concept generators a sanitized brief containing the user outcome, invariant product, data, safety, accessibility, brand, and implementation constraints, plus Krish's failure signals rewritten as non-solution requirements. Do not show generators the rejected renders, layout descriptions, rationales, change logs, rankings, or solution-specific features, and do not leak them through leading summaries. Give a fresh-context adversarial judge the new candidates, the sanitized brief, and the complete rejected history. Require the judge to detect repetition and regression, recover any proven strength during synthesis, and enforce the real constraints. Present Krish only the rendered synthesis unless comparing alternatives is itself his decision.
