---
name: krish-design
description: "Krish's design doctrine and house systems. Use before building or reshaping any visual artifact for Krish, including pages, one-pagers, PDFs, decks, invites, SVG, React UI, forms, flows, reskins, landing pages, and visual QA. Also trigger on requests to lay out, improve the look or UX, or create anything a person will see or touch. Exclude final copy voice (`krish-voice`), build mechanics (`krish-build`), and multi-phase app delivery (`build-apps-with-krish`). Covers interaction sequencing, house systems, material visual approval, anti-defaults, templates, end-of-build visual QA, and Canva handoff. Read `krish-principles` first."
---
