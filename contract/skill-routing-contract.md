...
| First user message equal to exactly `Video engine`, whitespace trimmed, nothing else | `video-engine` launcher | Latest `krishanraja/content-engine` `main` and its repository-owned workflow | `studio doctor`, job-list status, repository gates, and private-only publication policy |
...
| Harness governance: auditing, adding, revising, deprecating, quarantining, packaging or diagnosing a skill, rule, adapter or client surface | `mindmake-os` when the question touches live OS state | `harness-maintainer`, which owns skills, rules and adapters only. Fleet identity, architecture and database dependencies stay with `mindmake-os` under the collision rule below | Active-skill quality standard, staged before activation, and the approval gate for any upload, enable, disable or delete |
| A task needs an authenticated service: GitHub, Vercel, Supabase, n8n, Apify, Stripe or similar, or a login, scope, tenant or credential state fails | `tools-access` establishes access, identity, scope and target verification before any call | The reviewed per-tool skill, which is the single writer | Verified target and scope before the call, never a cached endpoint |
...
Provenance for the three rows added 2026-09-09, `harness-maintainer`, `tools-access` and `apify`: **Ruling (Krish, 2026-09-09)**, answering an `unrouted` finding open since 2026-09-08 and a proposal that was merged without being applied. Recorded here rather than in `brain/rules.yaml` because `ruleLines` in `scripts/brain-rules.mjs` extracts bullets and numbered items only, by design, so no table row in this contract carries machine-checked provenance. That gap is real and is Krish's to rule on: either the extractor learns tables and all 22 rows get backfilled, or table rows are declared routing rather than rules and this note is how they are sourced.
...
- `build-apps-with-krish` owns the stateful product journey across connected surfaces or multiple material app-delivery phases. `krish-design` still owns visual taste and the one-rendered-surface approval gate; `krish-build` owns implementation and, when a repeatable lifecycle earns it, the executable stage-conveyor topology inside the repository. Product phases and technical stages are different layers, not competing lifecycle owners. Route an isolated design, code, or QA task directly to its narrow owner. Generic app-building skills may supply a bounded mechanic but never replace this personalized orchestrator.
- `design-intelligence-search` is a delegation-gated, read-only subordinate of `krish-design` or `build-apps-with-krish`. It is discoverable so a structured delegation packet can reach it, but it fires only when the request explicitly names one of those two owners as delegator and names one bounded need for one concept arm. It may return pinned product, style, palette, type, landing, icon, chart, motion, or mobile-web candidates. It never triggers from a direct generic design/build/review request, never feeds every divergence arm, and never owns taste, current UX standards, code, persistence, QA, or accessibility claims. The upstream UI/UX Pro Max workflow, installer, broad auto-trigger language, persistence, and generic `ux`, `web`, `react`, and stack authority remain suppressed.
...
- `video-engine` activates only when the complete first user message, after trimming leading and trailing whitespace only, equals `Video engine` case-insensitively. A semantic client may retrieve the installed guard while resolving a nearby phrase; that read is not activation. Activation begins only after the exact check passes and the client crosses into the named authority repository or engine surface. Terminal punctuation, extra words or lines, quoted mentions, later-turn uses and `$video-engine` all fail and must not cross that boundary. Ordinary mentions of video, editing, captions, rendering, YouTube, Shorts, or a "video engine" remain with the applicable general capability. After a valid launch, the latest `krishanraja/content-engine` repository is authoritative and the installed skill remains a thin pointer. Narrowed 2026-09-08 to match the repository then named `krishanraja/mindmake-video-studio`: the canon had been granting a wider trigger than the source it deferred to. Retrieval and activation were distinguished explicitly in proposed PR 45 after a synthetic v2026.09.12.4 canary recorded a guard read with no observed access to that former repository identity. The 2026-09-15 rename changes the authority address, not what that older evidence proved. That is scoped evaluation evidence, not proof of real-world efficacy.
