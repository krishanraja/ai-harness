...
- **Installed and hash-verified on LORIMER's three local surfaces the same day.** `claude-code-user`, `cursor-primary` and `codex-current` each show 29 of 29 per-skill hashes exact against `v2026.09.13.8` in `state/skill-registry.yaml`, which now agrees with the machine's own deployment records for the first time since 2026-09-12.
- **`v2026.09.13.8`'s own live canary sheet is still not clean.** Claude Code reads `partial`: the externally-owned task route stays intentionally unmeasured. Codex reads `failed`: `instantly-operator` fired on a message that must not trigger it, holding the correct route in only 1 of 3 fresh attempts (`instantly-trigger-012`). Cursor stays `manual-required`, this client having no supported headless invocation evidence.
...
3. **Fix or rule on the one failure `v2026.09.13.8` still carries.** Codex loaded `instantly-operator` on a message that must not trigger it in 2 of 3 fresh attempts (`instantly-trigger-012`). The installed bytes are correct; a ninth release is the governed correction path, not a rewrite of the sheet.
...
- A `reviewed` date in `state/skill-registry.yaml` as proof of live behaviour rather than of a documentation check. All 29 skills read fresh as of 2026-09-13, and the release published the same day, `v2026.09.13.8`, still failed its own canary sheet on Codex.
