...
- **The trigger eval instrument is still invalid and its numbers are still suppressed.** Two controls over the same 120 cases. The first moved recall 0.250 between Haiku 4.5 and Sonnet 5; removing three routing rules the harness was injecting above the skill descriptions halved it to 0.125, and halved the accuracy delta from 0.150 to 0.075. It is still over the 0.1 bar and the stronger model still scores lower, so `audit-harness.mjs` still refuses every number. It refused a fix argued for on evidence, which is the only reason the bar is worth anything. The shape now points somewhere specific: precision is 1.000 for all five skills under both models while recall sits at 0.611 and 0.486, and perfect precision with poor recall is a router that will not fire rather than descriptions that are ambiguous. Next hypothesis is the framing, not the wording: a bare string with a JSON-only answer makes declining always the safe reply, and a real client sees a session. A control carries `router_prompt_sha256` so it can never vouch for a prompt it did not run on.
